...
# 5. 将坐标导出到CSV文件中
csv_file_path = 'd:/Code/route_coordinates3.csv'  # 保存的CSV文件路径
...
total_rows = sum(1 for row in open('route_coordinates3.csv')) - 1  # 减去标题行
target_df = pd.read_csv('route_coordinates3.csv')  
...
sheepdog1_position = [327, 1440]  # 第一只牧羊犬初始位置
sheepdog2_position = [327, 1440]  # 第一只牧羊犬初始位置
...
# 第一只牧羊犬驱赶函数
def drive_sheep(sheep_positions, sheepdog_position, index, initial_drive=True):
    ...
    if np.linalg.norm(direction_to_target) < 10:  # 如果接近目标位置，完成初始驱赶
        print(f"已到达初始位置, 羊群质心: {GCM}, 目标位置: {target_point}")
    ...
    if np.linalg.norm(abs(target_point-GCM)) < 10: # 如果绵羊距离全局中心质量的最大距离小于f_N
        index += 1
    
    return new_position, index, initial_drive
#第二只牧羊犬的驱赶
def drive_sheep_initial(sheep_positions, sheepdog_position,index):
    target_point = target_points[index]
    ...
    # 边界检查与反弹
    if new_position[0] < 0 or new_position[0] > L:
        new_position[0] = sheepdog_position[0] - δs * direction[0]  # 如果超出边界，反弹
    if new_position[1] < 0 or new_position[1] > L:
        new_position[1] = sheepdog_position[1] - δs * direction[1]  # 如果超出边界，反弹
...
def adjust_sheep_center_with_second_dog(sheep_positions, target_point, sheepdog2_position):
    ...
    new_position = sheepdog2_position + δs * direction  # 更新牧羊犬位置
    return new_position
#模拟函数
def simulate_shepherding(sheep_positions , previous_positions,sheepdog1_position, sheepdog2_position, max_steps=4000):
    positions = []
    ...
    for step in range(max_steps):
        ...
        if index >= len(target_points) - 2:
            ...
            sheepdog1_position, index, initial_drive = drive_sheep(sheep_positions, sheepdog1_position, index, initial_drive)
            if initial_drive==True:
                if np.max(np.linalg.norm(sheep_positions - GCM, axis=1)) < f_N:  # 如果绵羊距离全局中心质量的最大距离小于f_N
                    sheepdog2_position=drive_sheep_initial(sheep_positions,sheepdog2_position,index)
                else:
                    sheepdog2_position = collect_sheep(sheep_positions, sheepdog2_position)
            else :
                if np.linalg.norm(abs(target_point-GCM)) < 1:  # 如果羊群质心在轨迹附近
    ...
    return positions,previous_positions, sheepdog1_trajectory, sheepdog2_trajectory


# 运行模拟
positions,previous_positions, sheepdog1_trajectory, sheepdog2_trajectory = simulate_shepherding(sheep_positions, previous_positions,sheepdog1_position, sheepdog2_position)
...
def update(frame):
    ...
    return sheep_scatter, sheepdog1_scatter, sheepdog2_scatter
...
sheepdog1_position = [350, 1440]  # 第一只牧羊犬初始位置
sheepdog2_position = [327, 1440]  # 第一只牧羊犬初始位置
...
#模拟函数
def simulate_shepherding(sheep_positions , previous_positions,sheepdog1_position, sheepdog2_position, Flag,max_steps=10000):
    positions = []
    sheepdog1_trajectory = [sheepdog1_position.copy()]
    sheepdog2_trajectory = [sheepdog2_position.copy()]
    index = 0
    ...
    for step in range(max_steps):
        ...
        if index >= len(target_points) - 2:
            ...
            GCM = np.mean(sheep_positions, axis=0)  # 计算羊群的质心
            sheepdog1_position, index,flag = drive_sheep(sheep_positions, sheepdog1_position, index,flag)
            if np.linalg.norm(abs(target_point-GCM)) < 1:  # 如果羊群质心在轨迹附近
                sheepdog2_position = collect_sheep(sheep_positions, sheepdog2_position)  # 牧羊犬聚集绵羊
            else:#如果羊群质心偏离航线，需要让羊群回归正轨
                next_target_point = target_points[index]
        ...
        sheepdog1_trajectory.append(sheepdog1_position.copy())
        sheepdog2_trajectory.append(sheepdog2_position.copy())
    return positions,previous_positions, sheepdog1_trajectory, sheepdog2_trajectory,Flag

Flag=0
# 运行模拟
positions,previous_positions, sheepdog1_trajectory, sheepdog2_trajectory ,Flag= simulate_shepherding(sheep_positions, previous_positions,sheepdog1_position, sheepdog2_position,Flag)
print(f"时间步长：{Flag}")
csv_file_path ='d:/Code/Python/sheep_center_t.csv'
with open(csv_file_path,mode='w',newline='') as file:
    writer = csv.writer(file)
    writer.writerow(['x', 'y'])  # 写入表头
...
#将坐标导出到CSV文件中
csv_file_path = 'd:/Code/Python/sheepdog1.csv'  # 保存的CSV文件路径
with open(csv_file_path, mode='w', newline='') as file:
    writer = csv.writer(file)
    writer.writerow(['x', 'y'])  # 写入表头
    for n in zip(sheepdog1_trajectory):
        ...
        writer.writerow([x,y])
# 读取CSV文件
file_path2 = 'd:/Code/Python/sheepdog1.csv'
...
#将坐标导出到CSV文件中
csv_file_path = 'd:/Code/Python/sheepdog2.csv'  # 保存的CSV文件路径
with open(csv_file_path, mode='w', newline='') as file:
    writer = csv.writer(file)
    writer.writerow(['x', 'y'])  # 写入表头
    for n in zip(sheepdog2_trajectory):
        ...
        writer.writerow([x,y])
# 读取CSV文件
file_path3 = 'd:/Code/Python/sheepdog2.csv'
...
print(f"牧羊犬所走距离（平均值）：{(total_distance1+total_distance2)/2}")
total_row1 = sum(1 for row in open('route_coordinates.csv')) - 1  # 减去标题行
total_row2= sum(1 for row in open('sheep_center_t.csv'))-1
target_df = pd.read_csv('route_coordinates.csv')  
x1 = target_df['x'].to_numpy()
x1=np.array(x1)
y1 = target_df['y'].to_numpy()
y1=np.array(y1)
target_df = pd.read_csv('sheep_center_t.csv')  
