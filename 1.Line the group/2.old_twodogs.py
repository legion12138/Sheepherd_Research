...
#将坐标导出到CSV文件中
csv_file_path = 'd:/Code/sheepdog1.csv'  # 保存的CSV文件路径
with open(csv_file_path, mode='w', newline='') as file:
    writer = csv.writer(file)
    writer.writerow(['x', 'y'])  # 写入表头
    for n in zip(sheepdog1_trajectory):
        ...
        writer.writerow([x,y])
# 读取CSV文件
file_path2 = 'd:/Code/sheepdog1.csv'
...
if 'x' in df.columns and 'y' in df.columns:
    ...
    print("The file does not contain 'x' and 'y' columns.")
#将坐标导出到CSV文件中
csv_file_path = 'd:/Code/sheepdog2.csv'  # 保存的CSV文件路径
with open(csv_file_path, mode='w', newline='') as file:
    writer = csv.writer(file)
    writer.writerow(['x', 'y'])  # 写入表头
    for n in zip(sheepdog2_trajectory):
        ...
        writer.writerow([x,y])
# 读取CSV文件
file_path3 = 'd:/Code/sheepdog2.csv'
...
if 'x' in df.columns and 'y' in df.columns:
    ...
    # 计算相邻点之间的距离
    distances = np.sqrt(np.diff(x_coords)**2 + np.diff(y_coords)**2)
    print(np.diff(x_coords)**2)
    print(np.diff(y_coords)**2)
    # 计算总距离
    total_distance = np.sum(distances)
...
for i in range(1):
    ...
    print(f"时间步长：{Flag}")
    csv_file_path ='d:/Code/Python/sheep_center.csv'
    with open(csv_file_path,mode='w',newline='') as file:
        writer = csv.writer(file)
        writer.writerow(['x', 'y'])  # 写入表头
    ...
    total_row2= sum(1 for row in open('sheep_center.csv'))-1
    target_df = pd.read_csv('route_coordinates.csv')  
    x1 = target_df['x'].to_numpy()
    x1=np.array(x1)
    y1 = target_df['y'].to_numpy()
    y1=np.array(y1)
    target_df = pd.read_csv('sheep_center.csv')  
